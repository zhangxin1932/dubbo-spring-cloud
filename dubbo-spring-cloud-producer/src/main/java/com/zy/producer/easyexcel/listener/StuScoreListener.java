package com.zy.producer.easyexcel.listener;
...
public class StuScoreListener extends AnalysisEventListener<StuScoreExcel> {
    ...
    public StuScoreListener(StuScoreMapper stuScoreMapper) {
        this.stuScoreMapper = stuScoreMapper;
    }
    ...
    private void saveData() {
        ...
        try {
            stuScoreMapper.saveStuScore(list);
            log.info("存储数据库成功！");
        } catch (Exception e) {
            e.printStackTrace();
            log.error("存储数据库失败!");
